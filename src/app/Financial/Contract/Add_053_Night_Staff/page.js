...
const Night_Staff_Doc = (prop) => {
    ...
    const [Contractor_level, set_Contractor_level] = useState("a3") // Set Default Contractor_level for this page
    ...
    const calculateFinalPayableAmount = (record) => {
        ...
        const performanceWithholdingAmount = ((requested_performance_amount + overtime + shift) * performanceـwithholding_percentage) / 100;
        const vatAmount = ((requested_performance_amount + overtime + shift) * vat_percentage) / 100;
        const totalWorkAmount = (requested_performance_amount + overtime + vatAmount + shift);
        const taxAmount = ((requested_performance_amount + overtime + shift) * tax_percentage) / 100;
        const finalAmount = (totalWorkAmount - taxAmount - performanceWithholdingAmount);

        return {
            ...record,
            performanceـwithholding: performanceWithholdingAmount,
            ...
            render: (text, record) => {
                ...
                return (
                    <InputNumber
                        defaultValue={formattedValue}
                        variant="borderless"
                        onChange={(value) => handleInputChange(value, record, 'shift')}
                    />
                );
            },
        },
            ...
            render: (text, record) => {
                ...
                return (
                    <InputNumber
                        defaultValue={formattedValue}
                        variant="borderless"
                        onChange={(value) => handleInputChange(value, record, 'overtime')}
                    />
                );
            },
        },
...
export default Night_Staff_Doc;